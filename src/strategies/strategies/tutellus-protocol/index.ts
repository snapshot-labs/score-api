...
export async function strategy(
  ...
  const multiFarming = new Multicaller(network, provider, stakingABI, {
    blockTag
  });
  ...
  addresses.forEach(address =>
    multiStaking.call(
      address,
      '0x28Caa843cB577d892A8B6eC3F24Aa682ED22Be68',
      'getUserBalance',
      [address]
    )
  );
  ...
  addresses.forEach(address =>
    multiFarming.call(
      address,
      '0x57eB1b68F2ae0F77bf54F5EE6133bE80d6381d1B',
      'getUserBalance',
      [address]
    )
  );
  ...
  const [resultToken, resultStaking, resultFarming, resultPool] =
    await Promise.all([
      multiToken.execute(),
  ...
  const poolRatio = resultPool.reserves[0]
    .mul(parseEther('1'))
    .div(resultPool.totalSupply);

  const result: Record<string, BigNumberish> = {};

  addresses.forEach(address => {
    result[address] = resultToken[address]
      .add(resultStaking[address])
      .add(resultFarming[address].mul(poolRatio).div(parseEther('1')))
      .toString();
  });
