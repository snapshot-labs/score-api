...
async function stakingSubgraphQuery(
  ...
  const data = await subgraphRequest(subgraphUrl, query);
...
export async function strategy(
  ...
  const stakingSubgraphUrl = buildHoprStakeSubgraphUrl(
    options.variant ?? 'staking',
    options.season ?? 3
  );

  // trim addresses to sub of "LIMIT" addresses.
  const addressSubsets = [...Array(Math.ceil(addresses.length / LIMIT))].map(
    (_e, i) => addresses.slice(i * LIMIT, (i + 1) * LIMIT)
  );

  const returnedFromSubgraph = await Promise.all(
    addressSubsets.map(subset =>
      stakingSubgraphQuery(stakingSubgraphUrl, subset, subgraphBlock, snapshot)
    )
  );
